...
const params = {
  ...
  gridCols: 6,
  gridRows: 4,
  minSize: 1, // 最小グリッド数 -- Minimum number of grids
  gap: 8,
  cornerRadius: 16,
  speed: 1.0,
  renderBG: true,
...
function generateBento(forceReset = false) {
  if (forceReset) {
    ...
    boxes = []; // 強制リセット時はクリア -- Clear upon forced reset
  }
  randomSeed(params.seed);
  
  let layouts = [];
  
  // グリッドベースの分割 -- Grid-based division
  divideGrid(0, 0, params.gridCols, params.gridRows, params.depth, layouts);

  // グリッド座標をピクセル座標に変換 -- Convert grid coordinates to pixel coordinates
  const margin = 40;
  ...
  // layoutsにピクセル情報を付加 -- Add pixel information to layouts
  layouts.forEach(l => {
  ...
  // 1. ボックスの数を調整（足りなければ分裂させて追加）-- 1. Adjust the number of boxes (split them if there aren’t enough to add more)
  if (boxes.length < layouts.length) {
    let numToAdd = layouts.length - boxes.length;
    // 表示されているボックスから親候補を選ぶ -- Select a parent candidate from the boxes displayed
    ...
    for (let i = 0; i < numToAdd; i++) {
      ...
      let newBox;
      if (parent) {
        // 親の位置・サイズ・色を継承して生成（分裂表現）-- Generated by inheriting the parent’s position, size and colour (fission representation)
        newBox = new BentoBox(parent.x, parent.y, parent.w, parent.h);
        newBox.bgCol = parent.bgCol;
        newBox.fgCol = parent.fgCol;
      } else {
        // 親がいない場合は画面中央から -- If there are no parents, from the centre of the screen
  ...
  // 2. ターゲットの割り当て（距離が近い順にグリーディ法で割り当て）-- 2. Target allocation (allocated using the greedy algorithm in order of proximity)
  // layoutsのコピーを作成（割り当て済み管理用）-- Create a copy of “layouts” (for assigned administration)
  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    // 最も近い未使用レイアウトを探す -- Find the nearest unused layout
    ...
    if (bestLayoutIndex !== -1) {
      // 割り当て成功 -- Allocation successful
      availableLayouts[bestLayoutIndex].used = true;
      box.setTarget(availableLayouts[bestLayoutIndex]);
    } else {
      // 割り当てられなかったボックス（余剰分）は統合（消滅）させる -- Merge (eliminate) any unallocated boxes (surplus)
      // 最も近い「有効なレイアウト」の中心に向かって縮小させる -- Shrink them towards the centre of the nearest ‘valid layout’
...
function divideGrid(gx, gy, gw, gh, depth, layouts) {
  // 終了条件: 深さが0、または最小サイズ以下、またはランダム停止 -- Termination conditions: depth is 0, or the size is at or below the minimum size, or the algorithm stops randomly
  // 分割するには少なくとも minSize * 2 の幅が必要 -- A width of at least minSize * 2 is required to split
  ...
  // 分割方向の決定 -- Determining the direction of division
  let splitVertical = canSplitX;
  if (canSplitX && canSplitY) {
    // どちらも可能なら、長い方を優先しつつランダム性を持たせる -- If both are possible, prioritise the longer one whilst introducing an element of randomness
  ...
  if (splitVertical) {
    // 横幅を分割 (gw を splitW と gw - splitW に分ける) -- Split the width (divide gw into splitW and gw - splitW)
    // 分割点は minSize 以上残るように選ぶ -- Choose a split point that leaves at least minSize remaining
...
function updateLayout() {
  // 生きているボックスのみ対象 -- Applies only to active boxes
  let activeBoxes = boxes.filter(b => !b.isDead && b.tw > 0);
  
  // ボックス数に応じて分割/統合の確率を変える -- Adjust the probability of splitting or merging based on the number of boxes
  // 少ないときは分割優先、多いときは統合優先 -- Prioritise splitting when there are few boxes, and merging when there are many
  let splitProb = 0.5;
...
function splitBox(activeBoxes) {
  // 分割可能なボックス（最小サイズより大きい）を探す -- Find a divisible box (larger than the minimum size)
  ...
  // ピクセル計算用 -- For pixel calculations
  const margin = 40;
  const drawW = width - margin * 2;
  const drawH = height - margin * 2;
  const cellW = (drawW - (params.gridCols - 1) * params.gap) / params.gridCols;
  ...
  // ターゲットを更新（自分自身をl1に変形）-- Update target (transform self into l1)
  target.setTarget(l1);
  
  // 新しいボックスを追加（l2）-- Add a new box (l2)
  // 親の位置から発生させる -- Position relative to the parent
  let newBox = new BentoBox(target.x, target.y, target.w, target.h);
  newBox.bgCol = target.bgCol; // 色を継承 -- Inherit colours
...
function mergeBoxes(activeBoxes) {
  // 統合可能なペアを探す -- Find pairs that can be merged
  // 隣接していて、統合後に矩形になるペア -- Pairs that are adjacent and form a rectangle when merged
  let candidates = [];
  ...
  for (let i = 0; i < activeBoxes.length; i++) {
    for (let j = i + 1; j < activeBoxes.length; j++) {
      ...
      // 横に隣接 (同じY, 同じ高さ, Xが連続) -- Adjacent horizontally (same Y, same height, consecutive X values)
      if (b1.tgy === b2.tgy && b1.tgh === b2.tgh && (b1.tgx + b1.tgw === b2.tgx || b2.tgx + b2.tgw === b1.tgx)) {
        candidates.push({ b1, b2, type: 'horz' });
      }
      // 縦に隣接 (同じX, 同じ幅, Yが連続) -- Vertically adjacent (same X, same width, consecutive Y values)
  ...
  let newLayout;
  if (pair.type === 'horz') {
    // 左側をベースにする -- Use the left side as the base
    ...
    // 上側をベースにする -- Use the top side as the base
  ...
  // b2を消滅（サイズはそのままでフェードアウト）-- Make b2 disappear (fade out whilst retaining its size)
  b2.setTarget({ 
    x: b2.x, 
    y: b2.y, 
    w: b2.w, 
  ...
  // 飲み込む側(b1)を描画順の最前面（配列の末尾）に移動 -- Move the consuming side (b1) to the front of the rendering order (the end of the array)
...
function draw() {
  ...
  if (params.autoUpdate) {
    // 確率的に実行してタイミングを分散させる - Execute probabilistically to spread out the timing
  ...
  // 消滅したボックスを配列から削除 -- Remove the deleted box from the array
  boxes = boxes.filter(b => !b.isDead);

  // 書き出し処理 -- Export process
  if (isExporting) {
...
class BentoBox {
  constructor(x, y, w, h) {
    ...
    // ターゲット値（モーフィング用）-- Target values (for morphing)
    this.tx = x; this.ty = y; this.tw = w; this.th = h;
    // グリッド座標（初期値は0にしておく、setTargetで更新される）-- Grid coordinates (set to 0 initially; updated by setTarget)
    ...
    // コントラスト確保のため、白か黒を混ぜる -- To ensure good contrast, mix in some white or black
    this.adjustFgColor();
  ...
  adjustFgColor() {
    // パレットの色を使用するため、強制的な白黒設定は行わない -- To make use of the palette colours, do not force a black-and-white setting
  }
  ...
  initTypeData() {
    ...
    if (this.type === 'Graph') {
      ...
      this.data = [random(100, 999), random(100, 999)]; // 座標データ用 -- For coordinate data
    }
  }
  ...
  setTarget(layout) {
    ...
    // 新しい色とタイプを決定 -- Decide on the new colour and type
    if (layout.w > 0) {
      let colors = PALETTES[params.palette];
      let c1 = random(colors);
      ...
      // 一時的にfgColを更新して調整ロジックを通す -- Temporarily update fgCol and run it through the adjustment logic
  ...
  update() {
    ...
    // サイズがほぼ0になったら死亡フラグ -- If the size drops to almost zero, it’s a death flag
    if (this.tw === 0 && this.th === 0 && this.w < 1 && this.h < 1) {
      this.isDead = true;
    }

    // 吸収された場合はフェードアウトして消滅 -- If absorbed, it fades out and disappears
    if (this.isAbsorbed) {
      // フェードアウト表現は廃止。一定時間後に消滅させる -- The fade-out effect has been removed. The image will disappear after a set period of time.
      this.absorbedTimer++;
    ...
    if (this.type === 'Graph') {
      ...
      // タイプライターアニメーションの進行 -- Progress of the typewriter animation
      if (this.typewriterIndex < this.data[0].length) {
        this.typewriterIndex += 0.3 * params.speed;
      } else {
        // 完了後、たまにリセットして再入力 -- Once you’ve finished, occasionally reset and re-enter
  ...
  display() {
    ...
    // 本体 -- Main unit
    fill(this.bgCol);
    rect(this.x, this.y, this.w, this.h, 2); // 角丸を小さくしてシャープに -- Make the rounded corners smaller and sharper

    // コンテンツ描画（クリッピング領域内）-- Content rendering (within the clipping area)
    push();
    // クリップ（簡易的に矩形内描画）-- Clip (roughly drawn within a rectangle)
    ...
    fill(fg);
    stroke(fg);
    // 以降の描画関数で this.fgCol を使う場合も alpha が適用されるように、一時的に描画コンテキストの色を設定 -- Set the colour of the drawing context temporarily so that alpha is applied when using `this.fgCol` in subsequent drawing functions
  ...
  drawSwitch(cx, cy, s) {
    // 複数のトグルスイッチ風 -- Multiple toggle switch-style
    let count = 3;
    let swH = s * 0.2;
    let swW = s * 0.6;
    let gap = s * 0.1;
  ...
  drawTypography(cx, cy, s) {
    ...
    // カーソルと入力中のグリッチ -- Cursor and typing glitches
    if (len < txt.length) {
      if (frameCount % 10 < 5) displayTxt += "_"; // 点滅カーソル -- blinking cursor
      
      // 入力中の文字化け演出 -- Character corruption effect during input
      if (random() < 0.3 && len > 0) {
    ...
    // 完了後もたまに全体がグリッチする -- Even after completion, the whole thing occasionally glitches
  ...
  drawGraph(w, h) {
    ...
    // 最新値の表示 -- Display the latest values
    fill(c);
    textSize(10);
  ...
  drawCode(w, h) {
    ...
    for(let i=0; i<this.data.length; i++) {
      let txt = this.data[i];
      // ランダムに文字化けさせる -- Randomly garble the text
      if (random() < 0.05) {
  ...
  drawPattern(w, h) {
    ...
    for(let i=0; i<cols; i++) {
      for(let j=0; j<rows; j++) {
        // ノイズベースのパターン生成 -- Noise-based pattern generation
        let n = noise(i * 0.2, j * 0.2, time * 0.2 + this.x * 0.01);
        
        if (n > 0.45 && n < 0.55) {
           let size = cellSize * 0.8;
           if (random() < 0.05) size = cellSize; // グリッチ的にサイズ変化 -- グリッチ的にサイズ変化
  ...
  drawFuiDecorations(w, h) {
    ...
    // 装飾的なラインや矩形 -- Decorative lines and rectangles
    if (w > 60) {
        stroke(red(c), green(c), blue(c), 100);
